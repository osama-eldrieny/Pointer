...
// Single source of truth for comment statuses (backend). Add/remove one here and
// the stores partition files/keys accordingly. The browser component keeps its
// own copy (it's served standalone, no bundler) — keep the two in sync.
const STATUSES = ['open', 'pending-apply', 'applied'];
  ...
  if (method === 'PATCH' && (m = pathname.match(/^\/api\/comments\/([^/]+)$/))) {
    ...
    if (body.status) {
      if (!STATUSES.includes(body.status)) {
        return J(400, { error: `status must be one of: ${STATUSES.join(', ')}` });
      }
...
module.exports = { handleApi, sanitizeProject, STATUSES, DEFAULT_STATUS };
